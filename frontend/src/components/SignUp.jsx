  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate('/login'); // ⚡ 가입하면 로그인 주소로 보냄
      } else {
      ...
      <div style={{ width: '450px', padding: '30px', border: '1px solid #767676', borderRadius: '8px', backgroundColor: '#1a1a1e', color: '#ffffff', boxSizing: 'border-box' }}>
        ...
        <form onSubmit={handleSignUp}>
          <div style={{ marginBottom: '15px', textAlign: 'center' }}>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required style={{ width: '100%', padding: '12px', backgroundColor: '#3a3a3a', border: '1px solid #555555', borderRadius: '4px', color: '#ffffff', fontSize: '16px', boxSizing: 'border-box' }} />
          </div>
          <div style={{ marginBottom: '15px', textAlign: 'center' }}>
          ...
          <button type="submit" style={{ width: '100%', padding: '12px', backgroundColor: '#73b364', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '16px', cursor: 'pointer', marginBottom: '10px' }}>가입하기</button>
          <button type="button" onClick={() => navigate('/login')} style={{ width: '100%', padding: '12px', backgroundColor: '#444448', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '16px', cursor: 'pointer' }}>로그인 화면으로 돌아가기</button>
        </form>
      </div>
    </div>
  );
